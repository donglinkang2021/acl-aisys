...
\newtcolorbox{rolloutbox}[1]{
    ...
    title=#1,
    enhanced,
    breakable,            % [关键] 允许跨页，解决大部分留白问题
    before skip=2pt,      % 减小盒子前的间距
    after skip=2pt,       % 减小盒子后的间距
    attach title to upper,
    after title={\par\smallskip\hrule\smallskip}, % 减小标题下方的垂直间距
    coltitle=black,
    left=3pt,             % 进一步压缩内边距
    right=3pt,
    top=3pt,
...
% Prompt部分：同样加上 breakable 防止内部无法拆分
\newtcolorbox{promptpart}{
    ...
    fontupper=\small, % 移除加粗，因为代码已经用了 ttfamily，加粗会显得很臃肿并增加高度
    title=Prompt:,
...
\begin{abstract}
Diffusion Language Models (DLMs) offer significant advantages in parallel generation but suffer from the "Reload-and-Rollout" bottleneck during on-policy Reinforcement Learning (RL). This bottleneck stems from inefficient I/O and synchronization between training and inference engines, leading to substantial GPU underutilization. We propose \textbf{nanoRFT}, a unified framework featuring a "Twin-Engine" backend optimized for both Autoregressive and Diffusion architectures. By implementing an NCCL-based real-time parameter broadcast mechanism, nanoRFT reduces model synchronization latency to 10--50ms. Experimental results on the Reasoning task demonstrate that nanoRFT achieves a rollout throughput of 19,444 tokens/sec on NVIDIA H200 GPUs, representing a 21.76\% improvement over vLLM and up to 1.46$\times$ speedup over specialized DLM engines. Our framework provides a scalable and efficient baseline for the alignment and post-training of next-generation generative models.
\end{abstract}
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/nanorft.pdf} 
...
\begin{tabular}{@{}lcc@{}} 
\toprule
\textbf{Model (Architecture)} & \textbf{Disk I/O} & \textbf{nanoRFT (Ours)} \\ \midrule
Qwen2.5-1.5B (AR) & $\ge$3.0s & \textbf{$\sim$30ms} \\
Qwen2.5-3B (AR)   & $\ge$5.0s & \textbf{$\sim$30ms} \\ \midrule
SDAR-8B (DLM)     & $\ge$15.0s & \textbf{$\sim$10ms} \\
LLaDA-8B (DLM)    & $\ge$18.0s & \textbf{$\sim$10ms} \\ \bottomrule
\end{tabular}
\end{table}
...
To ensure the successful delivery of \textbf{nanoRFT}, the team adopted a cross-functional collaboration model. Each member contributed equally (25\%) across system design, algorithmic implementation, and experimental validation. The breakdown of responsibilities is summarized in Table~\ref{tab:collaboration}.
